   ...
   /**
    ...
    public function updateGuru($data) {
        ...
        try {
            ...
            // PERBAIKAN 1: Simpan hasil rowCount dari query pertama
            $rowCount = $this->db->rowCount();
            ...
            if ($guru && $guru['user_id']) {
                ...
                $this->db->bind(':user_id', $guru['user_id']);
                $this->db->execute();
                // Tambahkan rowCount dari query kedua jika ada perubahan
                $rowCount += $this->db->rowCount();
            }

            $this->db->commit();
            // PERBAIKAN 2: Kembalikan hasil rowCount yang sudah disimpan
            return $rowCount;
